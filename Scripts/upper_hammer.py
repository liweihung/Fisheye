...
class GeoAxes(Axes):
    ...
    def clear(self):
        ...
        self.set_longitude_grid_ends(74.86)
...
class UpperHammerAxes(HammerAxes):
	...
	def cla(self):
		HammerAxes.cla(self)
		Axes.set_xlim(self, -np.pi, 2*np.pi)
		Axes.set_ylim(self, 0, np.pi / 2.0)
		self.set_longitude_grid(30)
		self.set_latitude_grid(15)
		self.set_longitude_grid_ends(60)
	...
	def _set_lim_and_transforms(self):
		...
		yaxis_stretch = Affine2D().scale(np.pi*2, 1.02).translate(-np.pi, 0)
	...
	def set_longitude_grid(self, degrees):
		...
		ticking than set_xticks would.
		"""
		grid = np.linspace(-180, 180, int(360/degrees+1))
		self.xaxis.set_major_locator(FixedLocator(np.deg2rad(grid)))
		self.set_xticklabels(['N','30°','60°','E','120°','150°','S','210°','240°','W','300°','330°','N'])
	...
	def set_latitude_grid(self, degrees):
		...
		set_yticks would.
		"""
		grid = [0,13.83,29.11,44.51,59.71,74.86,90]
		self.yaxis.set_major_locator(FixedLocator(np.deg2rad(grid)))
		self.yaxis.set_major_formatter(self.ThetaFormatter(degrees))
