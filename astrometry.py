...
#originl image is save. Interim processing files are deleted. 
#
#Input: 
...
def main():
	...
	detail. All the inputs are defined and passed through filepath.
	"""
	...
	side = n.int(n.floor_divide(2*(xc-xmin),nsquare)) #length of each side [pix]
	...
	img = fits.open(filepath.data_cal+filepath.reference,uint=False)[0].data
	for i in xbound:
		for j in ybound:
			...
			fn = filepath.data_cal+'crop%i_%i.fit'%(i,j)
