...
#	(2) 'Test_Images/Coyote_Ridge_Astrometry/corr.fits' -- matched star list
#	(3) 'Test_Images/Coyote_Ridge_Astrometry/wcs.fits' -- wcs coordinates
#	(4) 'Test_Images/Coyote_Ridge_Astrometry/30sec.4_2x2_dark_subtracts_crop1400.fit'
#
...
hdu = fits.open('Test_Images/Coyote_Ridge_Astrometry/corr.fits')
...
hdu2 = fits.open('Test_Images/Coyote_Ridge_Astrometry/wcs.fits', fix=False)
w = WCS(hdu2[0].header)
...
imgname = '30sec.4_2x2_dark_subtracts_crop1400.fit'
hdu3 = fits.open('Test_Images/Coyote_Ridge_Astrometry/'+imgname, fix=False)
data = hdu3[0].data
...
for i in range(len(T)):	
	...
	sigma = abs(popt[2])          				       #sigma of the gaussian
	signal = popt[3]/ bg       						   #brightness/background

	if delta_position<1 and sigma<2 and signal>10:
...
T = T.round({'field_x':2,'field_y':2,'Flux':2,'Background':2})


#----------------------------
#Zeropoint and extinction

T['m'] = -2.5*n.log10(T.Flux)               #v_mag, apparent
...
#hdu3 = fits.open('Test_Images/Coyote_Ridge_Astrometry/wcs.fits')
